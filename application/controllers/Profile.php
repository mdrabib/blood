...
class Profile extends AdminController {
	...
	public function store(){
		...
		if ($this->form_validation->run() == FALSE){
			...
			if($this->cm->common_update('tbl_auth',$con,$ud)){
				/* set data to session again */
				$d=$this->db->where($con)->get('tbl_auth')->row();
				$this->session->set_userdata('ud',$d);

				$this->session->set_flashdata('msg','<b class="text-success text-center" style="font-size: xx-large">Data update</b>');
				redirect('profile');
			}else{
				$this->session->set_flashdata('msg','<b class="text-danger center" style="font-size: xx-large">Please Try again</b>');
				redirect('profile');
			}
        }
	}
